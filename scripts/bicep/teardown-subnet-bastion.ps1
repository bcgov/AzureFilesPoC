...
Write-Host "Tearing Down Bastion Subnet" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
...
# Check if subnet has any attached resources (especially Bastion hosts)
Write-Host ""
...
if ($attachedResources.nics -or $attachedResources.privateEndpoints) {
    ...
    if ($attachedResources.nics) {
        ...
        Write-Host "    (This may include a Bastion host - remove it first)" -ForegroundColor Yellow
    }
...
# Confirm deletion
Write-Host ""
Write-Host "⚠️  WARNING: This will delete the Bastion subnet." -ForegroundColor Red
Write-Host "Note: AzureBastionSubnet must be named exactly 'AzureBastionSubnet' to host a Bastion." -ForegroundColor Yellow
